...
def game(game_difficulty):
    ...
    while True:
        ...
        if not game_restarted:  # if the user doesn't press space bar, series will continue
            ...
            print(f"{username}'s Wins: {str(player_wins)}".ljust(
                16) + f"Computer's Wins: {str(computer_wins)}".rjust(20))
            print('\n' + " =" * 16 + '\n')
            time.sleep(0.5)
        else:
...
def main_loop():
    ...
    while True:
        ...
        series_winner(player_wins, computer_wins, match_count,
                      game_difficulty, series_count)
        next_game_trigger()
        clear()
